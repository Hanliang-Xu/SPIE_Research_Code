...
BIOCARD_output_path = Path('/nfs2/xuh11/Mean_Shift_BIOCARD_Multi')
        ...
        for run in folderpath.iterdir():
            if (run.name.startswith('run')):
                if (sub == "sub-JHU666199" and ses == "ses-160503"):
                    ...
                    applyConnectomeTransformation(input_file_2, output_file_2, mean1, mean2, multi_factor)
                    ...
                    applyConnectomeTransformation(input_file_1, output_file_1, mean1, mean2, multi_factor)
                else:
                    input_file_1 = folderpath / "run-1" / file_to_operate_on
                    output_path_1 = output_folder / sub / ses / "run-1"
                    subprocess.run(["mkdir", "-p", output_path_1], check=True)
...
target_cohorts(BIOCARD_df, BIOCARD_path, BIOCARD_output_path, "CONNECTOME_Weight_MEANLENGTH_NumStreamlines_10000000.csv", BIOCARD_Mean_Length_Mean, VMAP_Mean_Length_Mean, 2.0)
target_cohorts(BIOCARD_df, BIOCARD_path, BIOCARD_output_path, "CONNECTOME_Weight_NUMSTREAMLINES_NumStreamlines_10000000.csv", BIOCARD_Num_Streamlines_Mean, VMAP_Num_Streamlines_Mean, 1.1)