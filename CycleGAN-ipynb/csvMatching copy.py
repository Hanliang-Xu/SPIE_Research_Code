    ...
    while (not matched and j < BIOCARD_pd.shape[0]) :
        ...
        if (VMAP_sex == BIOCARD_sex and VMAP_sex == 2):
            while (not matched and j < BIOCARD_pd.shape[0] and BIOCARD_ID == BIOCARD_pd.iloc[j, 0]):
                ...
                if (abs(BIOCARD_age - VMAP_age) <= 0 and (VMAP_cog == "Normal")  and (BIOCARD_cog == "Normal") and (female_count < 44)):
                    if (female_count == 0 or female_count % 5 != 0) :
                        try:
        ...
        if (VMAP_sex == BIOCARD_sex and VMAP_sex == 1):
            while (not matched and j < BIOCARD_pd.shape[0] and BIOCARD_ID == BIOCARD_pd.iloc[j, 0]):
                ...
                if (abs(BIOCARD_age - VMAP_age) <= 0 and (VMAP_cog == "Normal")  and (BIOCARD_cog == "Normal")):
                    if (male_count == 0 or male_count % 5 != 0) :
                        print(male_count)
                        try:
                            df_concat = pd.concat([VMAP_pd.iloc[i], BIOCARD_pd.iloc[j]], axis=0).to_frame().T
                            final = pd.concat([final, df_concat], axis=0)
...
final.to_csv("/nfs2/xuh11/Connectome/CycleGAN/datasets/train_set_combined.csv", index=False)
final_BIO.to_csv("/nfs2/xuh11/Connectome/CycleGAN/datasets/train_set_A.csv", index=False)
final_VMAP.to_csv("/nfs2/xuh11/Connectome/CycleGAN/datasets/train_set_B.csv", index=False)