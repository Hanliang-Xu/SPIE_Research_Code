...
output_folder = Path('/nfs2/xuh11/Connectome/QA_BIOCARD/imagesInOneFolder')
        ...
        for run in session.iterdir():
            if (run.name.startswith('run')):
                for con in run.iterdir():
                    output_file_name = sub_folder.name + "_" + session.name + "_" + run.name + "_" + con.name
                ...
                output_file_name = sub_folder.name + "_" + session.name + "_" + run.name + "_" + con.name
                subprocess.run(['cp',con, output_folder])
                subprocess.run(['mv',output_folder/run.name, output_folder/output_file_name])