...
from modules import DetStepLoss, DetRNN, DetFODLoss, DetFCLoss, DetCNNFODCompress, DetConvProj, DetCNNFake, DetConvProjMulti
...
fod_cnn_file = os.path.join(out_dir, 'fod_cnn.pt')
fod_rnn_file = os.path.join(out_dir, 'fod_rnn.pt')
t1_cnn_file = os.path.join(out_dir, 't1_cnn.pt')
t1_rnn_file = os.path.join(out_dir, 't1_rnn.pt')
...
t1_cnn  = DetConvProjMulti(5, 48, 118, 512, kernel_size=7).to(device)
...
# fc_criterion  = DetStepLoss() # *** trying "end loss" to impose contrastive at output instead of in the middle
rnn_criterion = DetStepLoss()
mid_criterion = torch.nn.L1Loss()

opt0 = optim.Adam(list(fod_cnn.parameters()) + list(fod_rnn.parameters()),   lr=1e-3)
opt1 = optim.Adam(list(t1_cnn.parameters())  + list(t1_rnn.fc.parameters()), lr=1e-3)
opts = [opt0, opt1]
...
stage_max_epochs = 2000 # *** change as needed
total_max_epochs = (2-stage)*stage_max_epochs
stage_num_epochs_no_change = 200 # *** change as needed
